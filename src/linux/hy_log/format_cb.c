...
static hy_s32_t _format_color_cb(dynamic_array_s *dynamic_array, HyLogAddiInfo_s *addi_info)
{
    char buf[8] = {0};
    hy_s32_t ret = snprintf(buf, sizeof(buf), "%s", format_color[addi_info->level][1]);
    return dynamic_array_write(dynamic_array, buf, ret);
}

static hy_s32_t _format_level_str_cb(dynamic_array_s *dynamic_array, HyLogAddiInfo_s *addi_info)
{
    char buf[4] = {0};
    hy_s32_t ret = snprintf(buf, sizeof(buf), "[%s]", format_color[addi_info->level][0]);
    return dynamic_array_write(dynamic_array, buf, ret);
}

static hy_s32_t _format_time_cb(dynamic_array_s *dynamic_array, HyLogAddiInfo_s *addi_info)
{
    char buf[32] = {0};
    hy_s32_t ret = log_time(buf, sizeof(buf));
    return dynamic_array_write(dynamic_array, buf, ret);
}
...
static hy_s32_t _format_func_line_cb(dynamic_array_s *dynamic_array, HyLogAddiInfo_s *addi_info)
{
    ...
    if (addi_info->err_str) {
        ret += snprintf(buf + ret, sizeof(buf) - ret, "[%s:%"PRId32"]", addi_info->func, addi_info->line);
        ret += snprintf(buf + ret, sizeof(buf) - ret, "[errno: %d, err: %s] ", errno, addi_info->err_str);
    } else {
        ret += snprintf(buf + ret, sizeof(buf) - ret, "[%s:%"PRId32"] ", addi_info->func, addi_info->line);
    }
...
static hy_s32_t _format_usr_msg_cb(dynamic_array_s *dynamic_array, HyLogAddiInfo_s *addi_info)
{
    return dynamic_array_write_vprintf(dynamic_array, addi_info->fmt, addi_info->str_args);
}

static hy_s32_t _format_color_reset_cb(dynamic_array_s *dynamic_array, HyLogAddiInfo_s *addi_info)
{
    return dynamic_array_write(dynamic_array, PRINT_ATTR_RESET, strlen(PRINT_ATTR_RESET));
}
