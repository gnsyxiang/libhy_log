 ...
 */
#ifndef __LIBHY_LOG_INCLUDE_DYNAMIC_ARRAY_H_
#define __LIBHY_LOG_INCLUDE_DYNAMIC_ARRAY_H_
...
typedef struct {
    char        *buf;       ///< 存放数据的地方
    uint32_t    len;        ///< 开辟空间的大小
    uint32_t    cur_len;    ///< 已经存储数据的大小

    uint32_t    write_pos;  ///< 写位置
    uint32_t    read_pos;   ///< 读位置

    uint32_t    min_len;    ///< 开辟空间的最小长度
    uint32_t    max_len;    ///< 开辟空间的最大长度
} dynamic_array_s;
 ...
 * @return 返回实际读取到的长度
 */
int32_t dynamic_array_read(dynamic_array_s *context, void *buf, uint32_t len);
